...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        long nbr = teachers.stream().filter(n->n.getName().startsWith("s")).count();

        /* TO DO 2: Retourner la somme des salaires de tous les enseignants Flutter (hint: mapToInt) */
        long sum = teachers.stream().filter(n->n.getSubject().equals(Subject.FLUTTER)).mapToInt(Teacher::getSalary).sum();

        /* TO DO 3: Retourner la moyenne des salaires des enseignants dont le nom commence avec a */
        double average = teachers.stream().filter(n->n.getName().startsWith("a")).mapToInt(Teacher::getSalary).average().orElse(0.0);

        /* TO DO 4: Retourner la liste des enseignants dont le nom commence par f */
        List<Teacher> teachers1 = teachers.stream().filter(n->n.getName().startsWith("f")).toList();

        /* TO DO 5: Retourner la liste des enseignants dont le nom commence par s */
        List<Teacher> teachers2 = teachers.stream().filter(n->n.getName().startsWith("s")).toList();
        ...
        /*First way*/
        teachers.stream().filter(t->t.getSubject().equals(Subject.UNITY)).filter(t->t.getName().startsWith("g")).findFirst();
        /*Second way*/
        teachers.stream().filter(t->t.getSubject().equals(Subject.UNITY)).filter(t->t.getName().startsWith("g")).limit(1);

        /* TO DO 7: Afficher le deuxième enseignant dont le nom commence avec s */
        teachers.stream().filter(t->t.getName().startsWith("s")).limit(2).skip(1);
